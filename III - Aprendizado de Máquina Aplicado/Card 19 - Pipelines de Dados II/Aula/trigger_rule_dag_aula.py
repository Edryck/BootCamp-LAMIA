...
# Os raise ValueError("error") são para simular a task falhando
...
with DAG(dag_id='trigger_rule_dag', 
    ...
    # dag from the video
    # all_success - so roda se todas as tasks tiverem sucesso
    # all_failed - so roda se todas as tasks falharem
    # all_done - so roda se todas estiver pronto
    # one_failed - so roda se pelo menos uma taks falhar
    # one_success - so roda se pelo menos uma task ter sucesso
    # none_failed - so roda se nenhuma falhar
    # none_skipped - so roda se nenhuma for skipada    
    ...
    # o download_failed e ..._succeed depende das duas tasks de download
    # cada uma escuta um resultado diferente
    ...
    # process vai mandar duas notificacap em paralelo, elas decidem sozinhas se
    vai executar ou skippar pela trigger_rule
